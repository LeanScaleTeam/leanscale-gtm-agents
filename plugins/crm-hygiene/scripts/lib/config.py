...
#
# Prefers $CLAUDE_PLUGIN_ROOT when set so a Claude Code plugin update that
# relocates the cache cannot strand this file on a stale path.
BAKED={baked}
if [ -z "$1" ]; then
...
# Resolve the plugin directory: a valid $CLAUDE_PLUGIN_ROOT wins, else the baked path.
RESOLVED=
