...
/plugin install semantic-layer@leanscale-gtm
```
...
`~/.leanscale-gtm/profile.json` and `~/.leanscale-gtm/semantic-layer.json`.
...
|---|---|---|
| "No `crm.describe` tool resolved" | No CRM MCP server connected, or it exposes only write tools | Connect a Salesforce or HubSpot MCP server, then `/semantic-layer:setup --check` |
